...
describe("createTaskThread", () => {
  it("creates a thread with delegation source", async () => {
    ...
    const createCall = (ctx.db as unknown as { thread: { create: ReturnType<typeof vi.fn> } }).thread.create.mock
  ...
  it("generates a unique sourceId", async () => {
    ...
    const createCall = (ctx.db as unknown as { thread: { create: ReturnType<typeof vi.fn> } }).thread.create.mock
      .calls[0]?.[0] as { data: { sourceId: string } };
  ...
  it("sets lastActivity to a Date instance", async () => {
    ...
    const createCall = (ctx.db as unknown as { thread: { create: ReturnType<typeof vi.fn> } }).thread.create.mock
      .calls[0]?.[0] as { data: { lastActivity: unknown } };
