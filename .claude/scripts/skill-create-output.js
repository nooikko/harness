...
class SkillCreateOutput {
  ...
  header() {
    ...
    console.log(
      `${chalk.bold(chalk.magenta('║'))}     ${subtitle}${' '.repeat(Math.max(0, 59 - stripAnsi(subtitle).length))}${chalk.bold(chalk.magenta('║'))}`,
    );
    console.log(chalk.bold(chalk.magenta('╚════════════════════════════════════════════════════════════════╝')));
    console.log('');
  }
  ...
  instincts(instincts) {
    ...
    console.log(
      box(
        '🧠 Instincts Generated',
        instincts
          .map((inst, i) => `${chalk.yellow(`${i + 1}.`)} ${chalk.bold(inst.name)} ${chalk.gray(`(${Math.round(inst.confidence * 100)}%)`)}`)
          .join('\n'),
      ),
    );
  }
...
async function demo() {
  ...
  output.output('.claude/skills/pmx-patterns/SKILL.md', '.claude/homunculus/instincts/inherited/pmx-instincts.yaml');
