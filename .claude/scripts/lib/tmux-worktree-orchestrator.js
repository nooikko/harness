...
function buildWorkerArtifacts(workerPlan) {
  const seededPathsSection =
    workerPlan.seedPaths.length > 0 ? ['', '## Seeded Local Overlays', ...workerPlan.seedPaths.map((seedPath) => `- \`${seedPath}\``)] : [];
...
function buildOrchestrationPlan(config = {}) {
  ...
  const workerPlans = workers.map((worker, index) => {
    ...
    if (seenWorkerSlugs.has(workerSlug)) {
      ...
      throw new Error(`Worker names must map to unique slugs: ${workerSlug} (${firstWorkerName}, ${workerName})`);
    }
  ...
  const tmuxCommands = [
    ...
    {
      cmd: 'tmux',
      args: ['send-keys', '-t', sessionName, `printf '%s\\n' 'Session: ${sessionName}' 'Coordination: ${coordinationDir}'`, 'C-m'],
  ...
  for (const workerPlan of workerPlans) {
    tmuxCommands.push(
      ...
      {
        cmd: 'tmux',
        args: ['send-keys', '-t', '<pane-id>', `cd ${shellQuote(workerPlan.worktreePath)} && ${workerPlan.launchCommand}`, 'C-m'],
...
function cleanupExisting(plan) {
  ...
  for (const workerPlan of plan.workerPlans) {
    ...
    const existingWorktree = listWorktrees(plan.repoRoot).find((worktree) => worktree.canonicalPath === expectedWorktreePath);
...
function executePlan(plan) {
  ...
  runCommand('tmux', ['new-session', '-d', '-s', plan.sessionName, '-n', 'orchestrator', '-c', plan.repoRoot], { cwd: plan.repoRoot });
  runCommand(
    'tmux',
    ['send-keys', '-t', plan.sessionName, `printf '%s\\n' 'Session: ${plan.sessionName}' 'Coordination: ${plan.coordinationDir}'`, 'C-m'],
    { cwd: plan.repoRoot },
  );

  for (const workerPlan of plan.workerPlans) {
    const splitResult = runCommand('tmux', ['split-window', '-d', '-P', '-F', '#{pane_id}', '-t', plan.sessionName, '-c', workerPlan.worktreePath], {
      cwd: plan.repoRoot,
    });
    ...
    runCommand('tmux', ['send-keys', '-t', paneId, `cd ${shellQuote(workerPlan.worktreePath)} && ${workerPlan.launchCommand}`, 'C-m'], {
      cwd: plan.repoRoot,
    });
  }
