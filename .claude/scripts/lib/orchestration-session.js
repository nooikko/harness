...
function parseWorkerStatus(content) {
  ...
  for (const line of content.split('\n')) {
    ...
    if (key === 'worktree') {
      status.worktree = value;
    }
    if (key === 'taskfile') {
      status.taskFile = value;
    }
    if (key === 'handofffile') {
      status.handoffFile = value;
    }
  }
...
function parseWorkerHandoff(content) {
  return {
    ...
    validation: parseBullets(parseFirstSection(content, ['Validation', 'Tests / Verification', 'Tests', 'Verification'])),
    remainingRisks: parseBullets(parseFirstSection(content, ['Remaining Risks', 'Follow-ups', 'Follow Ups'])),
  };
}
...
function resolveSnapshotTarget(targetPath, cwd = process.cwd()) {
  ...
  if (fs.existsSync(absoluteTarget) && fs.statSync(absoluteTarget).isFile()) {
    ...
    const sessionName = normalizeSessionName(readPlanString(config, 'sessionName', absoluteTarget) || path.basename(repoRoot), 'session');
    const coordinationRoot = path.resolve(readPlanString(config, 'coordinationRoot', absoluteTarget) || path.join(repoRoot, '.orchestration'));
