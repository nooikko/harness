...
const AREA_PATTERNS: Record<string, RegExp[]> = {
  frontend: [/\/(app|pages|components|hooks|contexts|ui|views|layouts|styles)\//i, /\.(tsx|jsx|css|scss|sass|less|vue|svelte)$/i],
  backend: [/\/(api|routes|controllers|middleware|server|services|handlers)\//i, /\.(route|controller|handler|middleware|service)\.(ts|js)$/i],
  database: [
  ...
  integrations: [/\/(integrations?|third-party|external|plugins?|adapters?|connectors?)\//i, /\.(integration|adapter|connector)\.(ts|js)$/i],
  workers: [/\/(workers?|jobs?|queues?|tasks?|cron|background)\//i, /\.(worker|job|queue|task|cron)\.(ts|js)$/i],
};
...
function classifyFiles(allFiles: string[]): Record<string, AreaInfo> {
  ...
  for (const area of Object.values(areas)) {
    ...
    area.entryPoints = area.files.filter((f) => /index\.(ts|tsx|js|jsx)$/.test(f) || /main\.(ts|tsx|js|jsx)$/.test(f)).slice(0, 10);
  }
...
function generateAreaDoc(areaKey: string, area: AreaInfo, allFiles: string[]): string {
  ...
  const entrySection =
    area.entryPoints.length > 0 ? area.entryPoints.map((e) => `- \`${e}\``).join('\n') : '- *(no index/main entry points detected)*';
