...
analyze_observations() {
  ...
  # Sample observations: take last 500 lines to stay within prompt limits
  # (full file may be too large for a single prompt)
  obs_sample="$(tail -500 "$OBSERVATIONS_FILE" 2>/dev/null || echo "(no observations)")"

  cat > "$prompt_file" <<PROMPT
Analyze the following tool-use observations for the project ${PROJECT_NAME} and identify patterns (user corrections, error resolutions, repeated workflows, tool preferences).
...
PROMPT
  ...
  # Run claude from the homunculus project dir so it has read/write access to
  # instinct files and observations (avoids sandbox restrictions)
  (cd "$PROJECT_DIR" && claude --model haiku --max-turns "$max_turns" --print --dangerously-skip-permissions < "$prompt_file") >> "$LOG_FILE" 2>&1 &
  claude_pid=$!
