...
export const BudgetWarning: BudgetWarningComponent = ({ currentCost, budgetUsd }) => {
    ...
    return (
      <Card data-testid='budget-status'>
        <CardContent className='pt-6'>
          ...
          <div className='mt-2'>
            <Progress value={Math.min(usagePercent, 100) / 100} />
          </div>
  ...
  return (
    ...
    >
      ...
      <div className='mt-3'>
        <Progress value={Math.min(usagePercent, 100) / 100} className={!isCritical ? '*:data-[slot=progress-indicator]:bg-yellow-500' : undefined} />
      </div>
      <p className='mt-1 text-xs text-muted-foreground'>{usagePercent.toFixed(1)}% of budget used</p>
    </Alert>
  );
