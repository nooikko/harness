...
describe('ErrorDetailModal', () => {
  it('renders nothing when error is null', () => {
    const { container } = render(<ErrorDetailModal error={null} open={true} onOpenChangeAction={vi.fn()} />);
    expect(container.innerHTML).toBe('');
  });

  it('renders error message when open', () => {
    render(<ErrorDetailModal error={makeError()} open={true} onOpenChangeAction={vi.fn()} />);
  ...
  it('handles null stack gracefully', () => {
    render(<ErrorDetailModal error={makeError({ stack: null })} open={true} onOpenChangeAction={vi.fn()} />);
    expect(screen.getByText('Failed to play song')).toBeInTheDocument();
  });

  it('handles null threadId gracefully', () => {
    render(<ErrorDetailModal error={makeError({ threadId: null })} open={true} onOpenChangeAction={vi.fn()} />);
