...
export function calculateMonthEventPositions(multiDayEvents: IEvent[], singleDayEvents: IEvent[], selectedDate: Date): Record<string, number> {
  ...
  sortedEvents.forEach((event) => {
    ...
    // All-day events use exclusive end (RFC 5545). Timed events show only on their start day.
    const visualEnd = event.isAllDay ? new Date(rawEnd.getTime() - 1) : eventStart;
    const clampedStart = eventStart < monthStart ? monthStart : eventStart;
    const clampedEnd = visualEnd > monthEnd ? monthEnd : visualEnd;
...
export function getMonthCellEvents(date: Date, events: IEvent[], eventPositions: Record<string, number>) {
  ...
  const eventsForDate = events.filter((event) => {
    ...
    const rawEnd = parseISO(event.endDate);
    if (event.isAllDay) {
      // All-day events: exclusive end (RFC 5545) — back up 1ms so they don't bleed into the next day
      const visualEnd = new Date(rawEnd.getTime() - 1);
      return dayStart >= startOfDay(eventStart) && dayStart <= startOfDay(visualEnd);
    }
    // Timed events: only show on their start day
...
export const getEventsForDay = (events: IEvent[], date: Date, isWeek = false): (IEvent & { point?: 'start' | 'end' | 'none' })[] => {
  ...
  return events
    .filter((event) => {
      ...
      if (isWeek) {
        return event.startDate !== event.endDate && startOfDayForEventStart <= targetDate && startOfDayForEventEnd >= targetDate;
      }
