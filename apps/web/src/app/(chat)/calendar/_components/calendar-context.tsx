...
import type { TCalendarView, TEventColor } from '../_helpers/types';
...
type CalendarProviderComponent = (props: {
...
export const CalendarProvider: CalendarProviderComponent = ({ children, users, events, badge = 'colored', view = 'day' }) => {
  ...
  // Initialize from the prop (URL-driven) so server and client start with the same value.
  // Apply the persisted localStorage view after hydration to avoid mismatches.
  const [currentView, setCurrentViewState] = useState<TCalendarView>(view);
  useEffect(() => {
    setCurrentViewState(settings.view);
  }, []); // intentionally empty — runs once on mount to restore persisted view
...
type UseCalendar = () => ICalendarContext;
