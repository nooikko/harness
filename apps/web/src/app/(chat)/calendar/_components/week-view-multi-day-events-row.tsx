...
type WeekViewMultiDayEventsRowComponent = (props: IProps) => React.ReactNode;

export const WeekViewMultiDayEventsRow: WeekViewMultiDayEventsRowComponent = ({ selectedDate, multiDayEvents }) => {
  ...
  const processedEvents = useMemo(() => {
    return multiDayEvents
      .map((event) => {
        ...
        // All-day events use an exclusive end date (RFC 5545: ends at midnight of next day).
        // Subtract 1ms so the end index lands on the correct visual day.
        const visualEnd = event.isAllDay ? new Date(end.getTime() - 1) : end;
        const adjustedEnd = isAfter(visualEnd, weekEnd) ? weekEnd : visualEnd;
