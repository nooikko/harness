...
const sendDiscordMessage: SendDiscordMessage = async (
  ...
  if (!channel.isTextBased()) {
    ctx.logger.warn(
      `Cannot send message: channel "${channelId}" is not text-based`
    );
    return;
  }
...
const start: StartDiscordPlugin = async (ctx) => {
  ...
  if (!token) {
    ctx.logger.warn(
      "Discord plugin: DISCORD_TOKEN not set, skipping Discord connection"
    );
  ...
  client.on(Events.MessageCreate, async (message) => {
    ...
    ctx.logger.info(
      `Discord plugin: message from ${pipelineMsg.authorName} in ${pipelineMsg.channelName}`
    );
  ...
  try {
    ...
    ctx.logger.error(
      `Discord plugin: failed to connect: ${err instanceof Error ? err.message : String(err)}`
    );
    state.connected = false;
  }
};
...
export const getSendMessage: GetSendMessage = (pluginState, ctx) => {
  return async (sourceId: string, content: string) => {
    if (!pluginState.client || !pluginState.connected) {
      ctx.logger.warn(
        "Discord plugin: cannot send message, client not connected"
      );
      return;
    }
