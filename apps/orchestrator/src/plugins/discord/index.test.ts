...
const { mockOn, mockOnce, mockLogin, mockDestroy, mockFetchChannel } =
  vi.hoisted(() => ({
    mockOn: vi.fn(),
    mockOnce: vi.fn(),
    mockLogin: vi.fn(),
...
const findHandler: FindHandler = <T>(
  mock: ReturnType<typeof vi.fn>,
  eventName: string
) => {
...
const createMockContext = (
  overrides: Partial<PluginContext["config"]> = {}
): PluginContext => ({
  db: {
    thread: {
...
describe("discord plugin", () => {
  ...
  describe("start", () => {
    it("logs warning and skips connection when no token", async () => {
      ...
      expect(ctx.logger.warn).toHaveBeenCalledWith(
        expect.stringContaining("DISCORD_TOKEN not set")
      );
    });
    ...
    it("fires ClientReady handler to set connected state", async () => {
      ...
      expect(ctx.logger.info).toHaveBeenCalledWith(
        expect.stringContaining("connected as HarnessBot#1234")
      );
    });
    ...
    it("fires ShardDisconnect handler", async () => {
      ...
      expect(ctx.logger.warn).toHaveBeenCalledWith(
        expect.stringContaining("disconnected from gateway")
      );
    });
    ...
    it("fires ShardReconnecting handler", async () => {
      ...
      const handler = findHandler<ReconnectingHandler>(
        mockOn,
        "shardReconnecting"
      );
      expect(handler).toBeDefined();
      ...
      expect(ctx.logger.info).toHaveBeenCalledWith(
        expect.stringContaining("reconnecting to gateway")
      );
    });
    ...
    it("fires ShardResume handler", async () => {
      ...
      expect(ctx.logger.info).toHaveBeenCalledWith(
        expect.stringContaining("reconnected to gateway")
      );
    });
    ...
    it("processes a valid message through the pipeline", async () => {
      ...
      expect(
        ctx.db.thread.upsert as ReturnType<typeof vi.fn>
      ).toHaveBeenCalled();
      expect(
        ctx.db.message.create as ReturnType<typeof vi.fn>
      ).toHaveBeenCalled();
    ...
    it("handles database errors during message processing", async () => {
      mockLogin.mockResolvedValue("token");
      const ctx = createMockContext({ discordToken: "test-token" });
      (ctx.db.thread.upsert as ReturnType<typeof vi.fn>).mockRejectedValue(
  ...
  describe("getSendMessage", () => {
    ...
    it("warns when client is not connected", async () => {
      ...
      expect(ctx.logger.warn).toHaveBeenCalledWith(
        expect.stringContaining("client not connected")
      );
    });
