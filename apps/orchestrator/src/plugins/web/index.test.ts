...
describe("web plugin", () => {
  ...
  it("starts and stops the server gracefully", async () => {
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      expect.stringContaining("Web plugin listening")
    );
  ...
  it("serves REST endpoints after start", async () => {
    ...
    const listenCall = infoCalls.find(
      (call) =>
        typeof call[0] === "string" && call[0].includes("listening on port")
    );
  ...
  it("broadcasts WebSocket events via onBroadcast hook", async () => {
    ...
    await expect(
      hooks.onBroadcast?.("test:event", { data: "value" })
